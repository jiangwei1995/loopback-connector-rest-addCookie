...
RequestBuilder.prototype.invoke = function (parameters, cb) {
  ...
  // };
  function __getReqCookie(){
    var j = request.jar();
    var cookies = loopback.getCurrentContext().get('http').req.headers.cookie;
    if(cookies!==undefined){
      cookies.split(';').forEach(function (cookieStr){
        var cookie =request.cookie(cookieStr);
        j.setCookie(cookie, json.url);
      });
    }
    return j;
  };
  var __cookieJar = __getReqCookie();
  var req = {
    method: json.method,
    uri: json.url,
    qs: json.query,
  ...
  if (cb) {
    callback = function (err, response, body) {
      if (err) {
        ...
        var __res = loopback.getCurrentContext().get('http').res;
        __cookieJar.getCookies(req.uri).forEach(function (cookie){
          __res.cookie(cookie.key, cookie.value);
        });
        var result = body;
